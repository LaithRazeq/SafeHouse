...
def readHistory(v: str):
    log.set("")
    arr = thingspeak_read_h(READ_KEY,ID, v)
    history= ""
    for x in arr:
        for i in range(5):
            temp=""
...
def openHistory():
    ...
    Label(history,  
          text ="Select Number of History Entries", font="none 16 bold").pack()
    # Option meneu for choosing number of entries
    select = StringVar(history)
    select.set("1") # default value
    w = OptionMenu(history, select, "1","5", "10","15","20").place(x=300, y=30)    
    req=Button(history, text="Request", fg= "black", font= "none 12 bold", width=10, height=1, command= letsDoThis).place(x=360, y=30)   
    #History Labels     
